...
<select name="product_id" id="inputproduct_id" class="form-control" size="4" required data-name="{{ $products[0]->name }}" data-price="{{ $products[0]->price }}" data-qty="{{ $products[0]->qty }}">
	@foreach($products as $product)
		<option value="{{ $product->id }}" data-name="{{ $product->name }}" data-price="{{ $product->price }}" data-qty="{{ $product->qty }}">{{ $product->name }} ({{ indo_currency($product->price) }}), sisa {{ indo_currency($product->qty) }}</option>
	@endforeach
</select>
<script>
