			...
			<div class="col-sm-10 col-md-8 col-lg-6 mx-auto">
				<div class="card">
					<div class="card-header">
						Ubah Pengeluaran Nomor #{{ $expenditure->id }}
					</div>
					<div class="card-body">
						<form action="{{ route('expenditures.update', ['expenditure' => $expenditure->id]) }}" method="post" id="updateExpenditureForm" role="form">
							{{ csrf_field() }}
							{{ method_field('put') }}
							<div class="form-group row">
								<label for="inputdate" class="col-4 col-form-label">Tanggal</label>
								<div class="col-8">
									<input type="date" id="inputdate" name="date" value="{{ old('date', date('Y-m-d', strtotime($expenditure->creation_date))) }}" class="form-control" required {{ Auth::user()->level->name != 'Admin' ? 'readonly' : '' }} autocomplete="off">
									@if($errors->has('date'))
							...
							<div class="form-group row">
								...
								<div class="col-8">
									<textarea name="description" id="inputdescription" class="form-control" required autocomplete="off">{{ old('description', $expenditure->description) }}</textarea>
									...
									@foreach($types as $type)
										<div class="form-check">
											<label class="form-check-label">
												<input type="radio" class="form-check-input mt-3" name="type_id" value="{{ $type->id }}" {{ old('type_id', $expenditure->type_id) == $type->id ? 'checked' : '' }}>
							...
							<div class="form-group row">
								...
								<div class="col-8">
									<input type="text" id="inputprice" name="price" value="{{ old('price', indo_currency($expenditure->price)) }}" class="form-control" autocomplete="off">
									@if($errors->has('price'))
					...
					<div class="card-footer text-right">
						<button type="submit" id="submit-btn" form="updateExpenditureForm" class="btn btn-primary">
...
<script>
	...
	$("#updateExpenditureForm").submit(function() {
